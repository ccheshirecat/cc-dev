...
export function WalletDropdown({ onSelectCrypto }: WalletDropdownProps) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      setNotificationMessage(`@cheshirecat tipped you $${randomAmount} in ${randomCrypto.symbol}`)
      setShowNotification(true)

      // Update the balance for the tipped cryptocurrency
      ...
      // Update selected crypto if it was the one that received the tip
      if (selectedCrypto.symbol === randomCrypto.symbol) {
        setSelectedCrypto(prev => ({
          ...prev,
          balance: (parseFloat(prev.balance) + parseFloat(randomAmount)).toFixed(2)
        }))
      }
    }, 3000)
