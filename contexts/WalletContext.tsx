...
export interface CryptoBalance {symbol: string;
name: string;
balance: string;
}
...
const addRandomTip = useCallback(async () => {
  ...
  const randomIndex = Math.floor(Math.random() * state.balances.length);
  const randomCrypto = state.balances[randomIndex];
...
}, [state.hasAddedInitialTip, state.balances]);
